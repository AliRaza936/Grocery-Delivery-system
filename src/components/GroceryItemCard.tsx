...
function GroceryItemCard({item}:{item:IGrocery}) {
  ...
  const catrItem = cartData?.find(i=>i._id == item?._id)
  return (
    <motion.div
    ...
    >
      ...
      <div className='p-4 flex flex-col flex-1'>
        ...
        <h3 className='mb-auto'>{item?.name}</h3>
        ...
        {
          !catrItem ? <motion.button 
          className='mt-4 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white rounded-full py-2 text-sm font-medium transition-all cursor-pointer w-full'
          whileTap={{scale:0.96}}
        ...
        >
          ...
          <span className='text-sm w-5 text-center font-semibold text-gray-800'>{catrItem?.quantity}</span>
