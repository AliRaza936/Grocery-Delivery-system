'use client'
import { ArrowLeft, Minus, Plus, ShoppingBag, ShoppingBasket, Trash2 } from 'lucide-react'
    ...
    <span className='hidden sm:inline'>Back to home</span>
      ...
      >
        <ShoppingBasket className='w-16 h-16 text-gray-400 mx-auto mb-4'/>
          <p className='text-gray-600 text-lg mb-6 '>Your Cart is empty. Add some groceries to continue shoping</p>
          ...
          Continue Shoping
           ...
           <div className='grid grid-cols-1 lg:grid-cols-3 gap-8'>
            <div className='lg:col-span-2 space-y-5'>
              <AnimatePresence>
                {cartData.map((item,index)=>(
                  ...
                  >
                    <div className='relative w-28 h-28 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-xl overflow-hidden bg-gray-50 shrink-0'>
                      <Image 
                      src={item?.image} alt={item?.name} fill className='object-contain p-3 transition-transform duariton-300 hover:scale-105'
                      />
                    </div>
            ...
            <motion.div initial={{opacity:0,x:30}} animate={{opacity:1,x:0}} transition={{duration:0.3}} className='bg-white rounded-2xl shadow-xl p-6 h-fit sticky top-24 border border-gray-100 flex flex-col'>
              ...
              <motion.button whileTap={{scale:0.9}} className='w-full mt-6 bg-green-600 text-white py-3 rounded-full hover:bg-green-700 transition-all font-semibold text-sm sm:text-base cursor-pointer' onClick={()=>router.push('/user/checkout')}>
                  Proceed to Checkout
              </motion.button>
